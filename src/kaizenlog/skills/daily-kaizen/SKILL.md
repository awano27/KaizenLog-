...
### 今日の改善提案
1. [F#] （最大3個。観測数値は再掲せず「根拠ID→解釈→提案→翌日見る指標」の順で簡潔に）

### 明日の最小アクション
- [ ] [F#] （15分以内に始める行動）｜PASS: 翌日の数値条件｜FAIL: 未達条件
...
- 憶測で断定しない。各提案に根拠ID `[F#]`、対応する最小行動、PASS/FAILを1対1で付ける
- 観測数値は根拠IDの本文から書き写さない。新しい数値は最小アクションの行動量・PASS/FAIL、または「翌日」の目標だけに書く
