...
- `[F5]`: 構造化AIテレメトリ（Claude Code / Codex CLI）の `ai.sessions`、
  `fragmented`（中立の短セッション数）、`tool_errors`、`interruptions`、
  `retry_chains`、任意で `ai.sources` 内訳。項目がなければ測定不能
...
測定限界（分析中の参照。保存文には L-ID を出さなくてよい）:
- `[L13]`: 測定対象はこのPCの前景のみ。スマホ・他デバイス・離席中は含まれない。
...
### 明日試すこと
- [ ] （合図）→（15分以内に始める行動）｜PASS: context_switches <= 40（コンテキストスイッチ回数）｜FAIL: context_switches > 40
...
- Memoryにある未完了アクションを新しい提案として繰り返さない。
  ...
  **例外**: 実行済みFAILはより小さい一歩への分割継続を可とする
- 完了済み（status: done）の提案を蒸し返さない
- 拒否した提案はノートで `- [-] KZN-... ｜理由: ...` と書くか
  `kaizenlog skip KZN-... --reason ...` で記録する。スキップは消化率の分母から除外され、
  同種の再提案を避ける信号になる
...
- カテゴリ時間の減少を行動改善と断定しない（[L13]。PC外移行は未測定）
