...
4b. **実験の判定**: 実験ノートのうち `status: expired`（期限切れ）のものは、
   Measurementsの達成率・**効果量**・**同曜日基準**を材料に判定して
   frontmatter の status を書き換える:
   - 測定日の過半数で目標達成 **かつ** 効果量が意味のある方向（例: 削減目標で baseline 比
     -10%超の改善など）→ `status: adopted`（採用。習慣として定着したとみなす）
   - 達成率が過半数でも効果量が僅少（例: baseline 比 ±10%未満）なら「効果薄」として
     棄却を検討する（同曜日基準が目標側へ動いていない場合も同様）
   ...
   **風船効果**: PC前景のみ計測のため、カテゴリ時間の減少はスマホ等への移行の可能性を
   排除できない。体感と数値が合わない場合はデバイス移行を疑う。
