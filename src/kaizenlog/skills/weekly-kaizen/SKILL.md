...
4. **分析** — 日次では出せない以下の観点を重視する:
   ...
   - **AI作業の質のトレンド**: 往復数・細切れセッション・ツールエラー・中断の週次推移。
     改善しているか悪化しているか
   - **摩擦ワーストと入出力の質（LLM 判定は週次のみ）**:
     週次コンテキストの「⚠ 摩擦ワーストセッション」を一次データとし、
     同梱プロンプト `prompts/ai_work_deep_review.md` の観点
     （依頼の粒度・コンテキスト供給・資産化・中断パターン）で
     **入力プロンプトの質と出力の質を判定**する。
     日次の🧠セクションは決定論シグナル（内容抜粋・エラー・変更・末尾エラー）のみで、
     正しさや質の断定はしない（コストと捏造リスクのため）。
     判定結果はマーカー外に「AI活用スコアカード」として書く（例: 粒度 / コンテキスト /
     資産化 / 中断 の4軸を1〜5と1行根拠）。
   ...
   - **繰り返しプロンプト**: `kaizenlog prompts --unhandled --days 7` を実行し（使えなければ省略可）、
     頻出依頼があればテンプレ化/スキル化候補としてレビューに1〜3件記載する
   - **目標と実働の乖離パターン**: 週次コンテキストの「目標」節を一次データとし、
     目標を書いた日/書かない日の違い、目標カテゴリ実測分数と他カテゴリの関係を観察する
     （達成/未達の断定はしない。乖離の仮説はマーカー外の考察に書く）
