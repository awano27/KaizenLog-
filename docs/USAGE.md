...
提案の「### 明日の最小アクション」にはID付きチェックボックスが並びます：

```markdown
- [ ] KZN-20260707-001: [F9] 調査リンクを3件まとめる｜PASS: カテゴリ遷移5回以下｜FAIL: 6回以上
```

`[F9]` は提案に書かれた観測根拠のID、PASS/FAILは翌日の判定条件です。実行したら `[x]` にするだけ。**完了として記録され（Kaizen Memory）、LLMは同じ提案を繰り返さなくなります**。放置した提案も記録され、蒸し返しではなく「（継続）」として扱われます。
