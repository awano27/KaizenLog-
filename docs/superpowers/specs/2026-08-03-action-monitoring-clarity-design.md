...
「実行」と「観測」を同じcheckbox一覧へ混ぜず、1つの管理区間を3つの主ブロックと末尾の補助導線へ分ける。
...
- 週次提案件数、残件数、古い未完了件数、`kaizenlog today --all` は削除しない。
- これらは今日の実行カードより前へ置かず、3つの主ブロックを読んだ後の補助情報として末尾へまとめる。
- 完了率が低い場合の既存dosing契約と、責める表現を避ける既存reader UXを維持する。
...
5. 3つの主ブロックと末尾の状況・全件導線を `kaizenlog:actions` のMarkdownへレンダリングする。
