...
### 追加機能ストーリーへの改訂（ユーザー承認済み）

- デモの日次目標を、一般的なPRレビューから「KaizenLogの改善効果カード（Before / After）を日誌へ自動挿入する」へ変更する。
- サンプル内のアクション、効果モニタリング、Activity Log、洞察、日報、明日のフォーカスを同機能の設計・実装・検証へ統一する。
- 新機能、変更ファイル、コマンド、数値は架空であることを冒頭に明示し、現行実装済み機能との誤認を防ぐ。
- 改善効果カードはBefore / After、改善率、観測日数、PASS条件、証拠強度を表示し、小標本では因果未確定とする。
- 実行アクションは、期待Markdown4項目と失敗golden testを先に作る10分のマイクロアクションとし、checkboxは1件だけにする。

### v1.6.0リリース候補ストーリーへの改訂（ユーザー承認済み）

- 日次目標を「改善効果カードを含むKaizenLog v1.6.0のリリース候補を完成させる」へ変更する。
- 完了条件は、目玉機能、自動テスト、README/CHANGELOG、wheel/sdist、クリーン導入のRelease Check 5/5 PASSとする。
